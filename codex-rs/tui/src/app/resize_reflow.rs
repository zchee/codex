...
impl App {
    ...
    pub(super) fn reflow_transcript_now(&mut self, tui: &mut tui::Tui) -> Result<u16> {
        ...
        self.deferred_history_lines.clear();
        // Clear and rewrite scrollback in one synchronized write. Queueing the rewrite would
        // let a draw that observes a size change drop it after the terminal was already
        // cleared, permanently losing the transcript tail from scrollback.
        tui.replace_history_lines(reflowed_lines, self.history_line_wrap_policy())?;
