...
async fn tmux_split_after_final_message_keeps_it_in_scrollback() -> Result<()> {
    ...
    // Split as soon as the final message lands so the pane resize races the
    // finalize-time rebuild that clears and rewrites terminal scrollback.
    wait_for_capture_contains(
        &codex_pane,
        "resize reflow sentinel",
        Duration::from_secs(/*secs*/ 15),
    ...
    anyhow::ensure!(
        split_capture.contains("resize reflow sentinel"),
    ...
    anyhow::ensure!(
        restored_capture.contains("resize reflow sentinel"),
